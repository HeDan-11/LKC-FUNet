...
def ev_one(test_load, test_out_folder, dataset_name):
    MRI = dataset_name.split('-')[0]
    Other = dataset_name.split('-')[1]
    raw_MRI_path = test_load + f'/{MRI}/'
    raw_Other_path = test_load + f'/{Other}/'
    raw_MRI_list = glob(raw_MRI_path + f'/*')
    raw_Other_list = glob(raw_Other_path + f'/*')
    fused_imgs_list = glob(test_out_folder + f'/*')
    fused_imgs_list.sort()
    raw_MRI_list.sort()
    raw_Other_list.sort()
    # print(test_out_folder)
...
if __name__ == '__main__':
    ...
    ms = ['CT', 'PET', 'SPECT']
    for m in ms:
        fuse_root = f'E:/python/pytorch/Medical_image_fusion/data/test_imgs_IN'
